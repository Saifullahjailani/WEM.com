...
import { NavLinkCtx, NavLinks } from "../../NavLinkContextProvider";
  ...
  return (
    <FixedDiv>
      <Nav backgroundcolor={color || isMain ? "#282d32" : "transparent"}>
        ...
        <NavBtn>
          <Button to="/contacts/" primary="true" round="true">
            Contact Us
          </Button>
        </NavBtn>
      </Nav>
...
function BurgerMenu({
  ...
  return (
    <MenuPhone style={{ left: activate ? "0" : "-150vw" }}>
      {NavLinks.map((item: MenuItem, index) => (
        <NavLink handler={dispatch} to={item.link} key={item.link}>
          {item.title}
        </NavLink>
      ))}
      <Button
